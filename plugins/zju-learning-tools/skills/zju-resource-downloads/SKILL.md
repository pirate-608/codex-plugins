...
- Call `zju_list_resources` for one selected course.
- Call `zju_list_personal_resources` for the user's personal resource area.
...
- Use `zju_download_resource` for one upload.
- Use `zju_download_course_resources` only for the explicitly confirmed bounded set.
