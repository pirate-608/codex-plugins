...
- Academic periods: call `zju_list_terms`, then `zju_list_courses` with the selected term.
- Course discovery: call `zju_list_courses`, then `zju_get_course` only for the selected course.
- Upcoming work: call `zju_list_todos`; group by RFC 3339 deadline, course, and status.
- Course structure: call `zju_list_activities` for one selected course.
- Completion overview: call `zju_get_progress` for one selected course.
