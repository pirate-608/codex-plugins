...
$bundleSource = Join-Path $pluginRoot "autocad-plugin\CodexAutoCADMCP.bundle"
$applicationPlugins = Join-Path $env:APPDATA "Autodesk\ApplicationPlugins"
$bundleTarget = Join-Path $applicationPlugins "CodexAutoCADMCP.bundle"
...
Write-Output "Restart AutoCAD, then start a new Codex task and call system(operation='status')."