...
Install `autocad-mcp-codex` from the personal marketplace, then start a new Codex task so skills and MCP tools are reloaded. The server command in `.mcp.json` runs:
...
It synchronizes Python dependencies and copies `CodexAutoCADMCP.bundle` to `%APPDATA%\Autodesk\ApplicationPlugins`.
