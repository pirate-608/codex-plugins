...
description: Operate Adobe Premiere Pro through the bundled Premiere MCP integration with Windows app control as a fallback. Use when the user asks Codex to inspect, open, or create a Premiere project; import and organize media; edit sequences or timelines; trim clips; add transitions, effects, titles, captions, markers, or keyframes; mix audio; adjust color; diagnose the CEP bridge; validate an edit; or configure and export a video.
---

# Premiere Workflows

Use the bundled `premiere_pro` MCP tools as the primary structured editing interface. Use Windows computer control for CEP bridge setup, visual verification, and UI operations that the MCP cannot perform. Treat verified Premiere project state as the source of truth and preserve the user's existing edit.
...
3. Begin with read-only MCP discovery such as `get_project_info`, `list_sequences`, `get_active_sequence`, and `list_project_items`.
...
If the MCP tools are absent or the CEP panel must be configured, read the complete `computer-use` skill before Windows automation and follow its initialization, confirmation, and observation rules.
