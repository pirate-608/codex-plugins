...
- does not edit VS Code, Claude Desktop, or global Codex MCP configuration.
...
1. Read `premiere://config/get_instructions` when resources are available.
2. Inspect the project with `get_project_info`.
...
Prefer high-level tools such as `assemble_product_spot` only when the request matches their contract and all referenced assets are real and imported. For precise edits, use sequence-aware tools and pass `sequenceId` whenever supported.

## Exports

Run `validate_project_for_export` before export. Confirm the output path, preset, source range, and overwrite behavior. `export_sequence` requires a real readable Adobe Media Encoder `.epr` preset; do not invent a preset path. `get_render_queue_status` depends on Adobe Media Encoder integration.
...
`detect_silence` requires `ffmpeg` on `PATH` and analyzes source media directly; it does not read Premiere waveforms. Detection does not edit the timeline.
