...
`Codex -> stdio MCP -> photoshop-mcp-server -> photoshop-python-api -> Windows COM -> Adobe Photoshop`
...
Do not set `PS_VERSION` by default. `photoshop-python-api` can inspect the Windows registry and choose the newest registered Photoshop COM application. Define `PS_VERSION` only when deliberately targeting a supported installed major version, for example in the environment inherited by Codex.
