...
`photoshop_create_text_layer` can set:
...
Despite its name, `photoshop_create_solid_color_layer` does not create a native Photoshop Solid Color Fill content/adjustment layer. It creates a normal art layer, selects the full canvas, fills pixels with the requested RGB color, and then deselects.
...
1. Call `photoshop_get_selection_info`.
