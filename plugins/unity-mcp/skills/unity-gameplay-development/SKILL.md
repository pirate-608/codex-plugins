...
3. Verify Unity and package APIs with `unity_reflect`; use `unity_docs` when reflection or project assets do not answer the question.
...
9. Exercise the feature in Play mode and capture evidence when it has visual behavior.
...
- Use `execute_code` for bounded editor automation or inspection, not as a hidden replacement for maintainable project code.
- Add Undo support and mark assets/scenes dirty when custom editor code mutates serialized state.

## Completion Gate

Require successful compilation, no new console errors, passing targeted tests, observed behavior in Play mode when applicable, and saved scene/prefab assets.
