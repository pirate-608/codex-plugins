...
description: Orchestrate cross-domain Unity Editor work through Unity MCP tools and resources. Use for broad tasks spanning multiple Unity disciplines, initial MCP connection or instance selection, resource-first scene inspection, recovery from editor busy or domain-reload states, or requests that do not fit one specialized Unity workflow. Route focused setup, 2D, 3D, gameplay, UI, content, debugging, performance, or build work to the matching companion skill.
---
...
Use the narrowest matching companion skill and keep this orchestrator for shared MCP mechanics or cross-domain coordination:

| Request shape | Skill |
|---|---|
| Packages, tags, layers, pipelines, project readiness | `$unity-project-setup` |
| Sprites, 2D physics, 2D cameras, tile/grid levels | `$unity-2d-development` |
| 3D blockout, ProBuilder, lighting, materials, environment | `$unity-3d-worldbuilding` |
| C# gameplay, input, prefabs, architecture, feature tests | `$unity-gameplay-development` |
| UI Toolkit, uGUI, HUD, menus, responsive UI | `$unity-ui-design` |
| Textures, models, audio, animation, shaders, particles, VFX | `$unity-content-animation-vfx` |
| Console errors, regressions, failing tests, runtime bugs | `$unity-debug-testing` |
| CPU/GPU/memory/rendering measurement and optimization | `$unity-performance-optimization` |
| Build Profiles, player builds, release artifacts | `$unity-build-release` |

For a cross-domain request, apply the relevant specialized workflows in dependency order, sharing one editor-state and verification loop.
