...
7. Assemble reusable content into prefabs and instantiate prefabs through `manage_gameobject`.
8. Wait for imports and compilation, inspect the console, enter Play mode when runtime playback matters, and capture screenshots.
...
- Avoid large inline generated assets unless the user requested generation and the model/tool supports it.

## Verification Gate

Confirm the asset imports cleanly, references are not missing, the correct pipeline shader renders, animation/VFX/audio plays under the intended conditions, no new console errors appear, and visual content has screenshot evidence.
