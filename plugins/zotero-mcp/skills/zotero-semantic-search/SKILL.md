...
2. Call `zotero_get_search_database_status`. An empty database is not an MCP failure.
3. Build or incrementally refresh with `zotero_update_search_database`; use full text when local PDF
   content should be searchable. The deterministic CLI equivalent is
   `../../scripts/zotero-index.ps1 index`.
4. Query with `zotero_semantic_search`, then retrieve authoritative metadata and relevant source text
