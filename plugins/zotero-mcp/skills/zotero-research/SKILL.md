...
2. Start with `zotero_search_items`, tag search, citation-key search, or advanced search. Use
   `zotero_semantic_search` for conceptual queries only when the index is ready.
...
7. Use `zotero_export_bibliography` only after the final item set and citation style are clear.
